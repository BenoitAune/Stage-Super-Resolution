function [xhat, xinit] =  ggmm_epll(y,mask2,gamma,q,k_hat, sigma, prior_model, varargin)
...
Sigma=zeros(P*P,P*P,prior_model.GS.nmodels);
U=prior_model.GS.U;
S=prior_model.GS.S;
mu=zeros(P*P,prior_model.GS.nmodels);
for k=1:prior_model.GS.nmodels
     Sigma(:,:,k)= U{k}'*diag(S{k})*U{k};
     mu(:,k)=prior_model.GS.mu{k};
end 
% Core
xhat  = xinit;
[M, N] = size(xhat);
for iter = 1:num_iter
    ...
    % Reproject patches
    xtilde = get_image2D(zhat,P,M, N,k_hat,mu,Sigma, mask2,gamma);
