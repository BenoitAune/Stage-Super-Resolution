...
%im=rgb2gray(im);
perc=.3; %percentage of missing pixels
im=im2double(im);
im=im*255;
[Mim,Nim]=size(im);
...
imshow(imfi,[]);
colormap gray
ssim2=10*log10((mean(mean((im-imn).^2))/mean(mean((imfi-im).^2))))
psnr2=10*log10(255*255/mean(mean((im-imfi).^2)))

subplot(1,3,1);imagesc(im);title('image originale')
subplot(1,3,2);imagesc(imn);title('image masquée')
subplot(1,3,3);imagesc(imfi);title(sprintf('image reconstruite: PSNR %.1f SSIM %.3f',psnr2,ssim2 ));